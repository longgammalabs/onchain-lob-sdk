...
var getPriceDecimals = (price, priceScalingFactor) => {
  const wholePartDigits = price.integerValue(BigNumber4.ROUND_DOWN).toString().length;
...
var calculateBuyLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new BigNumber5(priceInput).dp(getPriceDecimals(BigNumber5(priceInput), priceScalingFactor), BigNumber5.ROUND_DOWN);
  const tokenXReceive = new BigNumber5(tokenXInput).dp(tokenXScalingFactor, BigNumber5.ROUND_DOWN);
  const minTokenYPay = price.times(tokenXReceive).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const maxFee = minTokenYPay.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const maxTokenYPay = minTokenYPay.plus(maxFee).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  return {
...
var calculateBuyLimitDetailsTokenYInput = (priceInput, tokenYInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new BigNumber5(priceInput).dp(getPriceDecimals(BigNumber5(priceInput), priceScalingFactor), BigNumber5.ROUND_DOWN);
  const maxTokenYPay = new BigNumber5(tokenYInput).dp(tokenYScalingFactor, BigNumber5.ROUND_FLOOR);
  const minTokenYPay = maxTokenYPay.div(new BigNumber5(1).plus(maxFeeRate)).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const tokenXReceive = minTokenYPay.div(price).dp(tokenXScalingFactor, BigNumber5.ROUND_DOWN);
  const maxFee = minTokenYPay.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateSellLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new BigNumber5(priceInput).dp(getPriceDecimals(BigNumber5(priceInput), priceScalingFactor), BigNumber5.ROUND_DOWN);
  const tokenXPay = new BigNumber5(tokenXInput).dp(tokenXScalingFactor, BigNumber5.ROUND_DOWN);
  const maxTokenYReceive = price.times(tokenXPay).dp(tokenYScalingFactor, BigNumber5.ROUND_FLOOR);
  const maxFee = maxTokenYReceive.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const minTokenYReceive = maxTokenYReceive.minus(maxFee).dp(tokenYScalingFactor, BigNumber5.ROUND_FLOOR);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateSellLimitDetailsTokenYInput = (priceInput, tokenYInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new BigNumber5(priceInput).dp(getPriceDecimals(BigNumber5(priceInput), priceScalingFactor), BigNumber5.ROUND_DOWN);
  const minTokenYReceive = new BigNumber5(tokenYInput).dp(tokenYScalingFactor, BigNumber5.ROUND_DOWN);
  const maxTokenYReceive = minTokenYReceive.div(new BigNumber5(1).minus(maxFeeRate)).dp(tokenYScalingFactor, BigNumber5.ROUND_FLOOR);
  const tokenXPay = maxTokenYReceive.div(price).dp(tokenXScalingFactor, BigNumber5.ROUND_UP);
  const maxFee = maxTokenYReceive.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateBuyMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestAsk, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  ...
  const tokenXReceive = new BigNumber6(tokenXInput).dp(tokenXScalingFactor, BigNumber6.ROUND_FLOOR);
  ...
  let worstPrice = new BigNumber6(bestAsk).times(new BigNumber6(1).plus(new BigNumber6(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber6.ROUND_CEIL);
  const tokenYPayWithoutFee = tokenXReceive.times(worstPrice).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  ...
  const [estTokenYPay, estFee] = calculateValueWithFee(
    new BigNumber6(estTokenYAmount),
    feeRate,
    tokenYScalingFactor
  );
  return {
    ...
    estPrice: ceil(estPrice, getPriceDecimals(BigNumber6(estPrice), priceScalingFactor)),
    estWorstPrice: ceil(estWorstPrice, getPriceDecimals(BigNumber6(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXReceive: tokenXReceive.toNumber(),
...
var calculateBuyMarketDetailsTokenYInput = (tokenYInput, maxSlippage, bestAsk, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenYPay = new BigNumber6(tokenYInput).dp(tokenYScalingFactor, BigNumber6.ROUND_FLOOR);
  ...
  let worstPrice = new BigNumber6(bestAsk).times(new BigNumber6(1).plus(new BigNumber6(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber6.ROUND_CEIL);
  const tokenXReceive = tokenYWithoutFee.div(worstPrice).dp(tokenXScalingFactor, BigNumber6.ROUND_FLOOR);
  const estTokenXReceive = new BigNumber6(estTokenXAmount).dp(tokenXScalingFactor, BigNumber6.ROUND_FLOOR);
  return {
    fee: fee.toNumber(),
    estFee: fee.toNumber(),
    worstPrice: worstPrice.toNumber(),
...
var calculateSellMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestBid, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenXPay = new BigNumber6(tokenXInput).dp(tokenXScalingFactor, BigNumber6.ROUND_FLOOR);
  ...
  let worstPrice = new BigNumber6(bestBid).times(new BigNumber6(1).minus(new BigNumber6(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber6.ROUND_FLOOR);
  const tokenYReceive = tokenXPay.times(worstPrice).dp(tokenYScalingFactor, BigNumber6.ROUND_FLOOR);
  ...
  return {
    ...
    estFee: estFee.toNumber(),
    worstPrice: worstPrice.toNumber(),
    estPrice: floor(estPrice, getPriceDecimals(BigNumber6(estPrice), priceScalingFactor)),
    estWorstPrice: floor(estWorstPrice, getPriceDecimals(BigNumber6(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXPay: tokenXPay.toNumber(),
...
var calculateSellMarketDetailsTokenYInput = (tokenYInput, maxSlippage, bestBid, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenYReceive = new BigNumber6(tokenYInput).dp(tokenYScalingFactor, BigNumber6.ROUND_FLOOR);
  ...
  let worstPrice = new BigNumber6(bestBid).times(new BigNumber6(1).minus(new BigNumber6(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber6.ROUND_FLOOR);
  const tokenXPay = tokenYReceiveBeforeFee.div(worstPrice).dp(tokenXScalingFactor, BigNumber6.ROUND_CEIL);
  const estTokenXPay = new BigNumber6(estTokenXAmount).dp(tokenXScalingFactor, BigNumber6.ROUND_CEIL);
  return {
    fee: fee.toNumber(),
    estFee: fee.toNumber(),
    worstPrice: worstPrice.toNumber(),
...
var calculateEstValuesFromTokenX = (tokenX, orderbookSide, initialPrice) => {
  let totalCost = new BigNumber6(0);
  let tokenXLeft = new BigNumber6(tokenX);
  let estWorstPrice = new BigNumber6(0);
  for (const level of orderbookSide) {
    const price = new BigNumber6(level.price);
    const size = new BigNumber6(level.size);
    const tradeQuantity = BigNumber6.min(tokenXLeft, size);
    totalCost = totalCost.plus(tradeQuantity.times(price));
    tokenXLeft = tokenXLeft.minus(tradeQuantity);
    estWorstPrice = price;
    if (tokenXLeft.lte(0)) break;
  }
  const estPrice = totalCost.div(new BigNumber6(tokenX).minus(tokenXLeft));
...
var calculateEstValuesFromTokenY = (tokenY, orderbookSide, initialPrice) => {
  let totalQuantity = new BigNumber6(0);
  let tokenYLeft = new BigNumber6(tokenY);
  let estWorstPrice = new BigNumber6(0);
  for (const level of orderbookSide) {
    const price = new BigNumber6(level.price);
    const size = new BigNumber6(level.size);
    const tradeCost = BigNumber6.min(tokenYLeft, size.times(price));
    totalQuantity = totalQuantity.plus(tradeCost.div(price));
    tokenYLeft = tokenYLeft.minus(tradeCost);
    estWorstPrice = price;
    if (tokenYLeft.lte(0)) break;
  }
  const estPrice = new BigNumber6(tokenY).minus(tokenYLeft).div(totalQuantity);
  const estSlippage = estWorstPrice.minus(initialPrice).div(initialPrice).abs().times(100);
...
var calculateValueWithFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  const valueWithFee = value.plus(fee).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  return [valueWithFee, fee];
};
var calculateValueAfterFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  const valueAfterFee = value.minus(fee).dp(tokenYScalingFactor, BigNumber6.ROUND_FLOOR);
  return [valueAfterFee, fee];
};
var calculateValueBeforeFee = (value, feeRate, tokenYScalingFactor) => {
  const valueBeforeFee = value.div(new BigNumber6(1).minus(feeRate)).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  const fee = valueBeforeFee.minus(value).dp(tokenYScalingFactor, BigNumber6.ROUND_CEIL);
  return [valueBeforeFee, fee];
};
var calculateAutoSlippage = (estSlippage) => {
  const diff = new BigNumber6(estSlippage).times(new BigNumber6(AUTO_SLIPPAGE_INCREASE_PERCENT).div(100));
  const autoSlippage = new BigNumber6(estSlippage).plus(diff).dp(AUTO_SLIPPAGE_DECIMAL, BigNumber6.ROUND_CEIL);
...
var getDepositDetails = ({
  ...
  let details = { tokenSpend: BigNumber9(0), lpReceive: BigNumber9(0), fee: BigNumber9(0), params: { amount: 0n, amountUsd: 0n, minLpMinted: 0n } };
  ...
  if (isLpTokenInput) {
    ...
    const tokenAmountUSDWithoutFee = lpAmount.times(totalValue).div(totalSupply).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR);
    const tokenAmountUSD = binarySearch(
      (x) => {
        const amountUSD = BigNumber9(x);
        const feeAmountUSD2 = calculateFeeAmountUSD(amountUSD).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR);
    ...
    const tokenSpend = tokenAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, BigNumber9.ROUND_FLOOR);
    const feeAmountUSD = BigNumber9(tokenAmountUSD).minus(tokenAmountUSDWithoutFee).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR);
    ...
    const amount = BigNumber9(tokenInput).dp(tokenDecimals, BigNumber9.ROUND_FLOOR);
    const amountUSD = amount.times(tokenPriceUSD).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR);
    const feeAmountUSD = calculateFeeAmountUSD(amountUSD).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR);
    const feeAmount = feeAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, BigNumber9.ROUND_FLOOR);
    const lpAmount = amountUSD.minus(feeAmountUSD).times(totalSupply).div(totalValue).dp(lpTokenDecimals, BigNumber9.ROUND_FLOOR);
    details = { ...details, tokenSpend: amount, lpReceive: lpAmount, fee: feeAmount };
  }
  details.params = getParams(
    details.tokenSpend,
    details.tokenSpend.times(tokenPriceUSD).dp(USD_DECIMALS, BigNumber9.ROUND_FLOOR),
    details.lpReceive,
    inputs.slippageBps,
    tokenDecimals,
    lpTokenDecimals
...
var getParams = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  const minLpMintedWithSlippage = lpAmount.times(BigNumber9(1).minus(slippage)).dp(lpTokenDecimals, BigNumber9.ROUND_FLOOR);
  return {
...
var getWithdrawDetails = ({
  ...
  const { tokenInput, lpInput } = inputs;
  const maxFeeBps = fee.feeBps.plus(fee.taxBps).plus(fee.adminBurnLPFeeBps).plus(fee.adminMintLPFeeBps);
  let details = { lpSpend: BigNumber10(0), tokenReceive: BigNumber10(0), fee: BigNumber10(0), params: { minTokenGet: 0n, minUsdValue: 0n, burnLP: 0n } };
  const calculateFeeAmountUSD = (amountUSD) => {
  ...
  if (isLpTokenInput) {
    const lpAmount = BigNumber10(lpInput).dp(lpTokenDecimals, BigNumber10.ROUND_FLOOR);
    const tokenAmountUSDWithoutFee = lpAmount.times(totalValue).div(totalSupply).dp(tokenDecimals, BigNumber10.ROUND_FLOOR);
    const tokenAmountUSD = tokenAmountUSDWithoutFee.div(BigNumber10(1).minus(maxFeeBps));
    const feeAmountUSD = BigNumber10(tokenAmountUSD).minus(tokenAmountUSDWithoutFee);
    const tokenReceive = tokenAmountUSDWithoutFee.div(tokenPriceUSD).dp(tokenDecimals, BigNumber10.ROUND_FLOOR);
    const feeAmount = feeAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, BigNumber10.ROUND_FLOOR);
    details = { ...details, lpSpend: lpAmount, tokenReceive, fee: feeAmount };
  } else {
    const amount = BigNumber10(tokenInput);
    const tokenAmountUSDWithoutFee = amount.times(tokenPriceUSD).dp(USD_DECIMALS, BigNumber10.ROUND_FLOOR);
    const tokenAmountUSD = binarySearch(
      (x) => {
        const amountUSD = BigNumber10(x);
        const feeAmount2 = calculateFeeAmountUSD(amountUSD).dp(tokenDecimals, BigNumber10.ROUND_FLOOR);
        ;
        return amountUSD.minus(feeAmount2).minus(tokenAmountUSDWithoutFee);
      },
      BigNumber10(0),
      tokenAmountUSDWithoutFee,
      tokenAmountUSDWithoutFee.times(BigNumber10(1).plus(maxFeeBps))
    );
    if (tokenAmountUSD === null) return details;
    const feeAmount = calculateFeeAmountUSD(BigNumber10(tokenAmountUSD));
    const lpAmount = tokenAmountUSDWithoutFee.times(totalSupply).div(totalValue).dp(lpTokenDecimals, BigNumber10.ROUND_FLOOR);
  ...
  details.params = getParams2(
    ...
    details.tokenReceive.plus(details.fee).times(tokenPriceUSD).dp(USD_DECIMALS, BigNumber10.ROUND_FLOOR),
    details.lpSpend,
    inputs.slippageBps,
    tokenDecimals,
    lpTokenDecimals
...
var getParams2 = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  const minTokenGetWithSlippage = amount.times(BigNumber10(1).minus(slippage)).dp(tokenDecimals, BigNumber10.ROUND_FLOOR);
  const minUsdValueWithSlippage = amountUSD.times(BigNumber10(1).minus(slippage)).dp(USD_DECIMALS, BigNumber10.ROUND_FLOOR);
  return {
