...
var getPriceDecimals = (price, priceScalingFactor) => {
  const wholePartDigits = price.integerValue(import_bignumber4.default.ROUND_DOWN).toString().length;
...
var calculateBuyLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new import_bignumber5.default(priceInput).dp(getPriceDecimals((0, import_bignumber5.default)(priceInput), priceScalingFactor), import_bignumber5.default.ROUND_DOWN);
  const tokenXReceive = new import_bignumber5.default(tokenXInput).dp(tokenXScalingFactor, import_bignumber5.default.ROUND_DOWN);
  const minTokenYPay = price.times(tokenXReceive).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  const maxFee = minTokenYPay.times(maxFeeRate).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  const maxTokenYPay = minTokenYPay.plus(maxFee).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  return {
...
var calculateBuyLimitDetailsTokenYInput = (priceInput, tokenYInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new import_bignumber5.default(priceInput).dp(getPriceDecimals((0, import_bignumber5.default)(priceInput), priceScalingFactor), import_bignumber5.default.ROUND_DOWN);
  const maxTokenYPay = new import_bignumber5.default(tokenYInput).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_FLOOR);
  const minTokenYPay = maxTokenYPay.div(new import_bignumber5.default(1).plus(maxFeeRate)).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  const tokenXReceive = minTokenYPay.div(price).dp(tokenXScalingFactor, import_bignumber5.default.ROUND_DOWN);
  const maxFee = minTokenYPay.times(maxFeeRate).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateSellLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new import_bignumber5.default(priceInput).dp(getPriceDecimals((0, import_bignumber5.default)(priceInput), priceScalingFactor), import_bignumber5.default.ROUND_DOWN);
  const tokenXPay = new import_bignumber5.default(tokenXInput).dp(tokenXScalingFactor, import_bignumber5.default.ROUND_DOWN);
  const maxTokenYReceive = price.times(tokenXPay).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_FLOOR);
  const maxFee = maxTokenYReceive.times(maxFeeRate).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  const minTokenYReceive = maxTokenYReceive.minus(maxFee).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_FLOOR);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateSellLimitDetailsTokenYInput = (priceInput, tokenYInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new import_bignumber5.default(priceInput).dp(getPriceDecimals((0, import_bignumber5.default)(priceInput), priceScalingFactor), import_bignumber5.default.ROUND_DOWN);
  const minTokenYReceive = new import_bignumber5.default(tokenYInput).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_DOWN);
  const maxTokenYReceive = minTokenYReceive.div(new import_bignumber5.default(1).minus(maxFeeRate)).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_FLOOR);
  const tokenXPay = maxTokenYReceive.div(price).dp(tokenXScalingFactor, import_bignumber5.default.ROUND_UP);
  const maxFee = maxTokenYReceive.times(maxFeeRate).dp(tokenYScalingFactor, import_bignumber5.default.ROUND_CEIL);
  return {
    price: price.toNumber(),
    maxFee: maxFee.toNumber(),
...
var calculateBuyMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestAsk, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  ...
  const tokenXReceive = new import_bignumber6.default(tokenXInput).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  ...
  let worstPrice = new import_bignumber6.default(bestAsk).times(new import_bignumber6.default(1).plus(new import_bignumber6.default(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), import_bignumber6.default.ROUND_CEIL);
  const tokenYPayWithoutFee = tokenXReceive.times(worstPrice).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  ...
  const [estTokenYPay, estFee] = calculateValueWithFee(
    new import_bignumber6.default(estTokenYAmount),
    feeRate,
    tokenYScalingFactor
  );
  return {
    ...
    estPrice: (0, import_lodash2.ceil)(estPrice, getPriceDecimals((0, import_bignumber6.default)(estPrice), priceScalingFactor)),
    estWorstPrice: (0, import_lodash2.ceil)(estWorstPrice, getPriceDecimals((0, import_bignumber6.default)(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXReceive: tokenXReceive.toNumber(),
...
var calculateBuyMarketDetailsTokenYInput = (tokenYInput, maxSlippage, bestAsk, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenYPay = new import_bignumber6.default(tokenYInput).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  ...
  let worstPrice = new import_bignumber6.default(bestAsk).times(new import_bignumber6.default(1).plus(new import_bignumber6.default(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), import_bignumber6.default.ROUND_CEIL);
  const tokenXReceive = tokenYWithoutFee.div(worstPrice).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  const estTokenXReceive = new import_bignumber6.default(estTokenXAmount).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  return {
    fee: fee.toNumber(),
    estFee: fee.toNumber(),
    worstPrice: worstPrice.toNumber(),
...
var calculateSellMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestBid, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenXPay = new import_bignumber6.default(tokenXInput).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  ...
  let worstPrice = new import_bignumber6.default(bestBid).times(new import_bignumber6.default(1).minus(new import_bignumber6.default(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), import_bignumber6.default.ROUND_FLOOR);
  const tokenYReceive = tokenXPay.times(worstPrice).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  ...
  return {
    ...
    estFee: estFee.toNumber(),
    worstPrice: worstPrice.toNumber(),
    estPrice: (0, import_lodash2.floor)(estPrice, getPriceDecimals((0, import_bignumber6.default)(estPrice), priceScalingFactor)),
    estWorstPrice: (0, import_lodash2.floor)(estWorstPrice, getPriceDecimals((0, import_bignumber6.default)(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXPay: tokenXPay.toNumber(),
...
var calculateSellMarketDetailsTokenYInput = (tokenYInput, maxSlippage, bestBid, orderbookSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  let autoSlippage = 0;
  let slippage = maxSlippage;
  const tokenYReceive = new import_bignumber6.default(tokenYInput).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  ...
  let worstPrice = new import_bignumber6.default(bestBid).times(new import_bignumber6.default(1).minus(new import_bignumber6.default(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), import_bignumber6.default.ROUND_FLOOR);
  const tokenXPay = tokenYReceiveBeforeFee.div(worstPrice).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_CEIL);
  const estTokenXPay = new import_bignumber6.default(estTokenXAmount).dp(tokenXScalingFactor, import_bignumber6.default.ROUND_CEIL);
  return {
    fee: fee.toNumber(),
    estFee: fee.toNumber(),
    worstPrice: worstPrice.toNumber(),
...
var calculateEstValuesFromTokenX = (tokenX, orderbookSide, initialPrice) => {
  let totalCost = new import_bignumber6.default(0);
  let tokenXLeft = new import_bignumber6.default(tokenX);
  let estWorstPrice = new import_bignumber6.default(0);
  for (const level of orderbookSide) {
    const price = new import_bignumber6.default(level.price);
    const size = new import_bignumber6.default(level.size);
    const tradeQuantity = import_bignumber6.default.min(tokenXLeft, size);
    totalCost = totalCost.plus(tradeQuantity.times(price));
    tokenXLeft = tokenXLeft.minus(tradeQuantity);
    estWorstPrice = price;
    if (tokenXLeft.lte(0)) break;
  }
  const estPrice = totalCost.div(new import_bignumber6.default(tokenX).minus(tokenXLeft));
...
var calculateEstValuesFromTokenY = (tokenY, orderbookSide, initialPrice) => {
  let totalQuantity = new import_bignumber6.default(0);
  let tokenYLeft = new import_bignumber6.default(tokenY);
  let estWorstPrice = new import_bignumber6.default(0);
  for (const level of orderbookSide) {
    const price = new import_bignumber6.default(level.price);
    const size = new import_bignumber6.default(level.size);
    const tradeCost = import_bignumber6.default.min(tokenYLeft, size.times(price));
    totalQuantity = totalQuantity.plus(tradeCost.div(price));
    tokenYLeft = tokenYLeft.minus(tradeCost);
    estWorstPrice = price;
    if (tokenYLeft.lte(0)) break;
  }
  const estPrice = new import_bignumber6.default(tokenY).minus(tokenYLeft).div(totalQuantity);
  const estSlippage = estWorstPrice.minus(initialPrice).div(initialPrice).abs().times(100);
...
var calculateValueWithFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  const valueWithFee = value.plus(fee).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  return [valueWithFee, fee];
};
var calculateValueAfterFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  const valueAfterFee = value.minus(fee).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_FLOOR);
  return [valueAfterFee, fee];
};
var calculateValueBeforeFee = (value, feeRate, tokenYScalingFactor) => {
  const valueBeforeFee = value.div(new import_bignumber6.default(1).minus(feeRate)).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  const fee = valueBeforeFee.minus(value).dp(tokenYScalingFactor, import_bignumber6.default.ROUND_CEIL);
  return [valueBeforeFee, fee];
};
var calculateAutoSlippage = (estSlippage) => {
  const diff = new import_bignumber6.default(estSlippage).times(new import_bignumber6.default(AUTO_SLIPPAGE_INCREASE_PERCENT).div(100));
  const autoSlippage = new import_bignumber6.default(estSlippage).plus(diff).dp(AUTO_SLIPPAGE_DECIMAL, import_bignumber6.default.ROUND_CEIL);
...
function safeSubtract(a, b) {
  return a.gt(b) ? a.minus(b) : (0, import_bignumber7.default)(0);
}
...
var binarySearch = (f, target, left, right, precision = 18) => {
  const step = (0, import_bignumber8.default)(10).pow(-precision);
  while (left.isLessThanOrEqualTo(right)) {
    const mid = left.plus(right).div(2).dp(precision, import_bignumber8.default.ROUND_FLOOR);
    const res = f(mid);
...
var getDepositDetails = ({
  ...
  let details = { tokenSpend: (0, import_bignumber9.default)(0), lpReceive: (0, import_bignumber9.default)(0), fee: (0, import_bignumber9.default)(0), params: { amount: 0n, amountUsd: 0n, minLpMinted: 0n } };
  ...
  if (isLpTokenInput) {
    ...
    const tokenAmountUSDWithoutFee = lpAmount.times(totalValue).div(totalSupply).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR);
    const tokenAmountUSD = binarySearch(
      (x) => {
        const amountUSD = (0, import_bignumber9.default)(x);
        const feeAmountUSD2 = calculateFeeAmountUSD(amountUSD).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR);
    ...
    const tokenSpend = tokenAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, import_bignumber9.default.ROUND_FLOOR);
    const feeAmountUSD = (0, import_bignumber9.default)(tokenAmountUSD).minus(tokenAmountUSDWithoutFee).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR);
    ...
    const amount = (0, import_bignumber9.default)(tokenInput).dp(tokenDecimals, import_bignumber9.default.ROUND_FLOOR);
    const amountUSD = amount.times(tokenPriceUSD).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR);
    const feeAmountUSD = calculateFeeAmountUSD(amountUSD).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR);
    const feeAmount = feeAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, import_bignumber9.default.ROUND_FLOOR);
    const lpAmount = amountUSD.minus(feeAmountUSD).times(totalSupply).div(totalValue).dp(lpTokenDecimals, import_bignumber9.default.ROUND_FLOOR);
    details = { ...details, tokenSpend: amount, lpReceive: lpAmount, fee: feeAmount };
  }
  details.params = getParams(
    details.tokenSpend,
    details.tokenSpend.times(tokenPriceUSD).dp(USD_DECIMALS, import_bignumber9.default.ROUND_FLOOR),
    details.lpReceive,
    inputs.slippageBps,
    tokenDecimals,
    lpTokenDecimals
...
var getParams = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  const minLpMintedWithSlippage = lpAmount.times((0, import_bignumber9.default)(1).minus(slippage)).dp(lpTokenDecimals, import_bignumber9.default.ROUND_FLOOR);
  return {
...
var getWithdrawDetails = ({
  ...
  const { tokenInput, lpInput } = inputs;
  const maxFeeBps = fee.feeBps.plus(fee.taxBps).plus(fee.adminBurnLPFeeBps).plus(fee.adminMintLPFeeBps);
  let details = { lpSpend: (0, import_bignumber10.default)(0), tokenReceive: (0, import_bignumber10.default)(0), fee: (0, import_bignumber10.default)(0), params: { minTokenGet: 0n, minUsdValue: 0n, burnLP: 0n } };
  const calculateFeeAmountUSD = (amountUSD) => {
  ...
  if (isLpTokenInput) {
    const lpAmount = (0, import_bignumber10.default)(lpInput).dp(lpTokenDecimals, import_bignumber10.default.ROUND_FLOOR);
    const tokenAmountUSDWithoutFee = lpAmount.times(totalValue).div(totalSupply).dp(tokenDecimals, import_bignumber10.default.ROUND_FLOOR);
    const tokenAmountUSD = tokenAmountUSDWithoutFee.div((0, import_bignumber10.default)(1).minus(maxFeeBps));
    const feeAmountUSD = (0, import_bignumber10.default)(tokenAmountUSD).minus(tokenAmountUSDWithoutFee);
    const tokenReceive = tokenAmountUSDWithoutFee.div(tokenPriceUSD).dp(tokenDecimals, import_bignumber10.default.ROUND_FLOOR);
    const feeAmount = feeAmountUSD.div(tokenPriceUSD).dp(tokenDecimals, import_bignumber10.default.ROUND_FLOOR);
    details = { ...details, lpSpend: lpAmount, tokenReceive, fee: feeAmount };
  } else {
    const amount = (0, import_bignumber10.default)(tokenInput);
    const tokenAmountUSDWithoutFee = amount.times(tokenPriceUSD).dp(USD_DECIMALS, import_bignumber10.default.ROUND_FLOOR);
    const tokenAmountUSD = binarySearch(
      (x) => {
        const amountUSD = (0, import_bignumber10.default)(x);
        const feeAmount2 = calculateFeeAmountUSD(amountUSD).dp(tokenDecimals, import_bignumber10.default.ROUND_FLOOR);
        ;
        return amountUSD.minus(feeAmount2).minus(tokenAmountUSDWithoutFee);
      },
      (0, import_bignumber10.default)(0),
      tokenAmountUSDWithoutFee,
      tokenAmountUSDWithoutFee.times((0, import_bignumber10.default)(1).plus(maxFeeBps))
    );
    if (tokenAmountUSD === null) return details;
    const feeAmount = calculateFeeAmountUSD((0, import_bignumber10.default)(tokenAmountUSD));
    const lpAmount = tokenAmountUSDWithoutFee.times(totalSupply).div(totalValue).dp(lpTokenDecimals, import_bignumber10.default.ROUND_FLOOR);
  ...
  details.params = getParams2(
    ...
    details.tokenReceive.plus(details.fee).times(tokenPriceUSD).dp(USD_DECIMALS, import_bignumber10.default.ROUND_FLOOR),
    details.lpSpend,
    inputs.slippageBps,
    tokenDecimals,
    lpTokenDecimals
...
var getParams2 = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  const minTokenGetWithSlippage = amount.times((0, import_bignumber10.default)(1).minus(slippage)).dp(tokenDecimals, import_bignumber10.default.ROUND_FLOOR);
  const minUsdValueWithSlippage = amountUSD.times((0, import_bignumber10.default)(1).minus(slippage)).dp(USD_DECIMALS, import_bignumber10.default.ROUND_FLOOR);
  return {
