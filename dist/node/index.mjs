...
import { Contract, ContractTransactionResponse, Signature, Transaction, TransactionResponse } from "ethers";
...
var _OnchainLobSpotMarketContract = class _OnchainLobSpotMarketContract {
  constructor(options) {
    ...
    __publicField(this, "syncSendRawTransaction");
    __publicField(this, "customSignTransaction");
  ...
  async processContractMethodCall(contract, method, args) {
    ...
    if (this.syncSendRawTransaction) {
      try {
        ...
        if (error.code === "UNSUPPORTED_OPERATION" || error.message?.includes("eth_signTransaction") || error.error?.message?.includes("Method not supported")) {
          ...
          if (error.data) {
            try {
              ...
              if (parseError instanceof TransactionFailedError) throw parseError;
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async processContractMethodCallSync(contract, method, args) {
    const methodName = method.fragment?.name ?? method.name ?? "unknown";
    ...
    if (populatedTx.nonce === void 0 || populatedTx.nonce === null || populatedTx.nonce === 0) {
      stepStart = Date.now();
    ...
    stepStart = Date.now();
    const signedTx = this.customSignTransaction ? await this.customSignTransaction(populatedTx) : await this.signer.signTransaction(populatedTx);
    ...
    stepStart = Date.now();
    const result = await provider.send("eth_sendRawTransactionSync", [signedTx, "pending"]);
    const hash = typeof result === "string" ? result : result.transactionHash;
...
var getPriceDecimals = (price, priceScalingFactor) => {
  const priceUnits = parseUnits2(price.toFixed(priceScalingFactor), priceScalingFactor).toString();
...
var calculateBuyLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  const price = new BigNumber4(priceInput).dp(getPriceDecimals(BigNumber4(priceInput), priceScalingFactor), BigNumber4.ROUND_DOWN);
  ...
  const maxFee = minTokenYPay.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber4.ROUND_CEIL);
...
var calculateSellLimitDetailsTokenXInput = (priceInput, tokenXInput, priceScalingFactor, tokenXScalingFactor, tokenYScalingFactor, maxFeeRate) => {
  ...
  const maxFee = maxTokenYReceive.times(maxFeeRate).dp(tokenYScalingFactor, BigNumber4.ROUND_CEIL);
...
var getMarketDetails = ({ market, orderbook, inputToken, inputs, direction }) => {
  ...
  const bestAsk = asks[0]?.[0] ? Number(formatUnits2(asks[0][0], market.priceScalingFactor)) : 0;
  const bids = orderbook.bids.sort((a, b) => Number(BigInt(b[0]) - BigInt(a[0])));
  const bestBid = bids[0]?.[0] ? Number(formatUnits2(bids[0][0], market.priceScalingFactor)) : 0;
  if (direction === "buy") {
    if (inputToken === "base") {
...
var calculateBuyMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestAsk, asksSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  ...
  let worstPrice = new BigNumber5(bestAsk).times(new BigNumber5(1).plus(new BigNumber5(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber5.ROUND_CEIL);
  ...
  return {
    ...
    estPrice: ceil(estPrice, getPriceDecimals(BigNumber5(estPrice), priceScalingFactor)),
    estWorstPrice: ceil(estWorstPrice, getPriceDecimals(BigNumber5(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXReceive: tokenXReceive.toNumber(),
...
var calculateSellMarketDetailsTokenXInput = (tokenXInput, maxSlippage, bestBid, bidsSide, tokenXScalingFactor, tokenYScalingFactor, priceScalingFactor, feeRate, useAutoSlippage) => {
  ...
  let worstPrice = new BigNumber5(bestBid).times(new BigNumber5(1).minus(new BigNumber5(slippage).div(100)));
  worstPrice = worstPrice.dp(getPriceDecimals(worstPrice, priceScalingFactor), BigNumber5.ROUND_FLOOR);
  ...
  return {
    ...
    estFee: estFee.toNumber(),
    worstPrice: worstPrice.toNumber(),
    estPrice: floor(estPrice, getPriceDecimals(BigNumber5(estPrice), priceScalingFactor)),
    estWorstPrice: floor(estWorstPrice, getPriceDecimals(BigNumber5(estWorstPrice), priceScalingFactor)),
    estSlippage,
    autoSlippage,
    tokenXPay: tokenXPay.toNumber(),
...
var calculateEstValuesFromTokenX = (tokenX, orderbookSide, initialPrice, priceScalingFactor, tokenXScalingFactor) => {
  ...
  for (const [rawPrice, rawSize] of orderbookSide) {
    const price = new BigNumber5(formatUnits2(rawPrice, priceScalingFactor));
    const size = new BigNumber5(formatUnits2(rawSize, tokenXScalingFactor));
...
var calculateValueWithFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const valueWithFee = value.plus(fee).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  return [valueWithFee, fee];
};
var calculateValueAfterFee = (value, feeRate, tokenYScalingFactor) => {
  const fee = value.times(feeRate).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const valueAfterFee = value.minus(fee).dp(tokenYScalingFactor, BigNumber5.ROUND_FLOOR);
  return [valueAfterFee, fee];
};
var calculateValueBeforeFee = (value, feeRate, tokenYScalingFactor) => {
  const valueBeforeFee = value.div(new BigNumber5(1).minus(feeRate)).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
  const fee = valueBeforeFee.minus(value).dp(tokenYScalingFactor, BigNumber5.ROUND_CEIL);
...
var OnchainLobSpot = class {
   ...
   */
  setSigner(signer, options) {
    this.signer = signer;
    if (options?.customSignTransaction !== void 0) {
      this.customSignTransaction = options.customSignTransaction;
    }
...
var getParams = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  ...
  return {
    amount: parseUnits3(amount.toString(), tokenDecimals),
    amountUsd: parseUnits3(amountUSD.toString(), USD_DECIMALS),
    minLpMinted: parseUnits3(minLpMintedWithSlippage.toString(), lpTokenDecimals)
  };
};
...
var getWithdrawDetails = ({
  ...
  if (isLpTokenInput) {
    ...
    const tokenAmountUSD = tokenAmountUSDWithoutFee.div(BigNumber9(1).minus(maxFeeBps));
    const feeAmountUSD = BigNumber9(tokenAmountUSD).minus(tokenAmountUSDWithoutFee);
...
var getParams2 = (amount, amountUSD, lpAmount, slippage, tokenDecimals, lpTokenDecimals) => {
  ...
  return {
    burnLP: parseUnits4(lpAmount.toString(), lpTokenDecimals),
    minTokenGet: parseUnits4(minTokenGetWithSlippage.toString(), tokenDecimals),
    minUsdValue: parseUnits4(minUsdValueWithSlippage.toString(), USD_DECIMALS)
  };
};

// src/vault/onchainLobVaultContract.ts
import { Contract as Contract2, ContractTransactionResponse as ContractTransactionResponse2, Transaction as Transaction2, TransactionResponse as TransactionResponse2 } from "ethers";
...
var _OnchainLobVaultContract = class _OnchainLobVaultContract {
  ...
  async processContractMethodCallSync(contract, method, args) {
    ...
    const signedTx = this.customSignTransaction ? await this.customSignTransaction(populatedTx) : await this.signer.signTransaction(populatedTx);
    console.log(`[onchain-lob-sdk] syncSend [${methodName}] sign (${this.customSignTransaction ? "custom" : "signer"}): ${Date.now() - stepStart}ms`);
    const txObj = Transaction2.from(signedTx);
    stepStart = Date.now();
    const result = await provider.send("eth_sendRawTransactionSync", [signedTx, "pending"]);
    const hash = typeof result === "string" ? result : result.transactionHash;
    console.log(`[onchain-lob-sdk] syncSend [${methodName}] eth_sendRawTransactionSync: ${Date.now() - stepStart}ms`);
    console.log(`[onchain-lob-sdk] syncSend [${methodName}] TOTAL: ${Date.now() - totalStart}ms, hash: ${hash}`);
    const baseTxResponse = new TransactionResponse2({
      blockNumber: null,
      blockHash: null,
      hash: hash ?? txObj.hash,
      index: 0,
    ...
    return new ContractTransactionResponse2(contract.interface, provider, baseTxResponse);
  }
  convertTokensAmountToRawAmountIfNeeded(amount, decimals) {
    return typeof amount === "bigint" ? amount : tokenUtils_exports.convertTokensAmountToRawAmount(amount, decimals);
  }
...
var OnchainLobVault = class {
  ...
  setSigner(signer, options) {
    ...
    this.vaultContracts = /* @__PURE__ */ new Map();
    return this;
  }
  /**
